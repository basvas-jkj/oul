...
namespace oul
{
    /// @brief Variadické omezení povolující pouze argumenty převoditelné na <code>std::string</code> nebo <code>boost::filesystem::path</code>.
    ...
    /// @return <code>true</code>, pokud nástroj úspěšně doběhl, <code>false</code> v opačném případě.
    export template <CmdArgs ...T>
    bool call_tool(cr<path> tool_path, cr<path> working_directory, T&&... args)
    {
        child ch(tool_path, std::forward<T>(args)..., start_dir(working_directory), std_out > null, std_err > null);
        ch.wait();
